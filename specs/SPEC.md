...
* Arguments that name requirement nodes, graph nodes, workspace files, or file globs (`<node>`, `<graph-node>`, `<file>`, `--file`) are workspace-relative, independent of the working directory. `<node>` and `<graph-node>` values are identities in the form of 1.5, their `#` splitting path from id or unit; the split applies equally to an operand spelled `<file>#<id>` (6.5). A bare `<file>` operand and a `--file` glob are a whole path or pattern: `#` has no delimiter role in them, so a `#`-containing spelling names the discovered file of that invalid path (14.19, 11.4), never a `path#id` pair. `--config <path>` and `--test-hold <path>` are filesystem paths resolved against the working directory.
